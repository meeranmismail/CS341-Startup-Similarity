...
class CompanyCorpus(object):
    ...
    def filter_desc_by_idf(self, description_column_name, number_words_to_cut):
        ...
        for desc in self.corpus.loc[:, description_column_name]:
            ...
            if iteration % 10000 is 0 or (iteration+1) is self.corpus.shape[0]:
                if (iteration+1) is self.corpus.shape[0]:
                    iteration += 1
                sys.stdout.write('\r')
                sys.stdout.write("Filter by IDF Percent Complete: {0:0.2f}%"
                                 .format(math.ceil((iteration/self.corpus.shape[0])*100)))
                sys.stdout.flush()
            iteration += 1
        sys.stdout.write('\n')
        ...
        for company in self.company_corpus.corpus.iterrows():
            ...
            if iteration % 10000 is 0 or (iteration+1) is graph_size:
                if (iteration+1) is graph_size:
                    iteration += 1
                sys.stdout.write('\r')
                sys.stdout.write("LSH Forest Build Percent Complete: {0:0.2f}%"
                                 .format(round((iteration/graph_size)*100)))
                sys.stdout.flush()
            iteration += 1
        sys.stdout.write('\n')
        ...
        for company1 in self.company_corpus.corpus[self.company_name_column_name]:
            ...
            if iteration % 10000 is 0 or (iteration+1) is graph_size:
                if (iteration+1) is graph_size:
                    iteration += 1
                sys.stdout.write('\r')
                sys.stdout.write("Graph Build Percent Complete: {0:0.2f}%"
                                 .format(round((iteration/graph_size)*100)))
                sys.stdout.flush()
            iteration += 1
        sys.stdout.write('\n')
